...
async def run_oneoff():
    ...
    config = SimpleNamespace(app_store_count=1250)
    
    logging.info("Starting one-off ingestion for App Store (1250 items)...")
    raw_items = await connector.fetch(config=config)
    logging.info(f"Fetched {len(raw_items)} items from App Store.")
    if not raw_items:
        return
