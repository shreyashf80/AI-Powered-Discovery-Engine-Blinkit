...
import { FileText, Loader2, Target, BarChart2, Lightbulb } from "lucide-react";
  ...
  return (
    <div className="flex flex-col min-h-screen max-w-4xl mx-auto py-12 px-6">
      <div className="flex items-center justify-between mb-10">
        <div>
          <h1 className="text-3xl font-sans font-semibold text-ink mb-2">Insight Summary Report</h1>
        ...
        <div className="flex flex-col items-end gap-2">
          {summary?.generated_at && (
            <span className="text-[11px] font-mono bg-surface border border-surface px-3 py-1.5 rounded text-ink-muted whitespace-nowrap">
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-accent text-accent-ink px-5 py-2.5 rounded-md font-medium hover:opacity-90 transition-opacity disabled:opacity-50 shrink-0"
          >
      ...
      {summary && (
        <div className="space-y-12">
          ...
          {summary.emergent_themes && summary.emergent_themes.length > 0 && (
            <div className="bg-gradient-to-br from-accent/10 to-transparent border border-accent/20 rounded-xl p-8">
          ...
          <div>
            ...
            <div className="space-y-8">
              {summary.summaries.map((item, idx) => (
                <div key={idx} className="bg-surface border border-surface rounded-xl p-6 hover:shadow-sm transition-shadow">
                  <div className="flex justify-between items-start mb-4 gap-4">
                    ...
                    <div className="flex flex-col items-end gap-1 shrink-0">
                      <span className={`text-[11px] font-mono uppercase px-2 py-1 rounded-sm border ${
                        item.confidence === 'High' ? 'bg-positive/10 text-positive border-positive/20' :
                        item.confidence === 'Medium' ? 'bg-[#D4A017]/10 text-[#D4A017] border-[#D4A017]/20' :
                        'bg-negative/10 text-negative border-negative/20'
                      }`}>
                        {item.confidence} CONFIDENCE
                      </span>
                      <span className="text-[11px] font-mono text-ink-muted">
                        N = {item.sample_size} sources
                      </span>
                    </div>
                  </div>
